...
void loop() {
  ...
  if ( digitalRead(boton1) == LOW  && banderaBTN1 == 0) {
    ...
    contador = humedad ;
    banderaBTN1 = 1;
    //int temperaturaDisplay = temperatura;
    LCD_Print(String(unidad), 60, 150, 2, 0x001F, 0xCAB9);
    LCD_Print(String(decimal),  75, 150, 2,  0x001F, 0xCAB9);
    LCD_Print("% rH", 90, 150, 2, 0x001F, 0xCAB9);
...
void comunicacionESP(){
  if(Serial2.available()){//si hay algun dato esperando a ser leido...
    humedad = Serial2.read();//el dato es guardado en la variable
    }  
  }
